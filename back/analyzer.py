...
def analyze_momentum(data):
    ...
    momentum = {
        ...
        "rsi_signal": (
            "SOBRECOMPRA"
            if rsi.iloc[-1] > 70
            else ("SOBREVENTA" if rsi.iloc[-1] < 30 else "NEUTRAL")
        ),
        "macd": macd.iloc[-1],
...
def full_analysis(data, name="Activo"):
    ...
    if trend.get("sma_20"):
        color = "green" if trend.get("sobre_sma20") else "red"
        console.print(f" SMA 20:: [{color}]{trend['sma_20']:.2f}[/{color}]")

    if trend.get("sma_50"):
        color = "green" if trend.get("sobre_sma50") else "red"
        console.print(f"   SMA 50: [{color}]{trend['sma_50']:.2f}[/{color}]")

    if trend.get("sma_200"):
        color = "green" if trend.get("sobre_sma200") else "red"
        console.print(f"   SMA 200: [{color}]{trend['sma_200']:.2f}[/{color}]")
